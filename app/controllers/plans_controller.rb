class PlansController < ApplicationController
    ...
    csv_string = FasterCSV.generate do |csv|
      ...
      @projects.each do |project|
        if project.exists_in_agilefant?
          ...
          project.paths.tagged_as('1.3m').each do |path|
  ...
  #            end
  #          else
              csv << [3, "T-#{path.id}", (path.name)[0..max_project_length], nil, nil, nil, nil, nil, nil, nil, application, nil, nil, release]
              path.fragments.each do |fragment|
                ap fragment
                ap fragment.af_user
                csv << [4, "T-#{fragment.id}", (path.name)[0..max_project_length], fragment.hours_equivalent, nil, fragment.build_iteration.start_date.strftime("%m/%d/%Y"), fragment.build_iteration.stop_date.strftime("%m/%d/%Y"), fragment.af_user.fullName, nil, nil, application, nil, nil, release]
#              else # unplanned
